...
void	ft_permutation(unsigned short tetri[26][13], int start, int end, int *square)
{
	...
	*/	/*ft_printcombination(tetri);*/
		...
		*/
	/*	printf("permutation done calling solver\n");
	*/	ft_bzero(&tetri[25][1], 24);
		ft_solvecomb(tetri, 0, square);
	...
	else
	{
		...
		while (i <= end)
		{
	/*		if (ft_comparetetri(&tetri[start][1], &tetri[i][1]) == 1)
				i++;*/
			ft_swapbitmap(tetri[start], tetri[i]);

			ft_permutation(tetri, start + 1, end, square);
	/*		if (ft_comparetetri(&tetri[start][1], &tetri[i][1]) == 0)
	*/			ft_swapbitmap(tetri[start], tetri[i]);
